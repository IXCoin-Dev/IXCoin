...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in ixc/kb\n"
...
Value sendalert(const Array& params, bool fHelp)
{
    ...
    // if key is not correct openssl may crash, comment taken from other coin source. not sure it's valid now in v0.9.2.1?
    // Added runtime_error on SetPrivKey returning false...
    ...
    else if (!key.Sign(Hash(alert.vchMsg.begin(), alert.vchMsg.end()), alert.vchSig))
