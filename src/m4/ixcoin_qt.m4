...
dnl Output: If qt version is auto, set ixcoin_enable_qt to false. Else, exit.
AC_DEFUN([IXCOIN_QT_FAIL],[
  if test "x$ixcoin_qt_want_version" = "xauto" && test x$ixcoin_qt_force != xyes; then
    if test x$ixcoin_enable_qt != xno; then
      AC_MSG_WARN([$1; ixcoin-qt frontend will not be built])
    fi
    ixcoin_enable_qt=no
  else
    AC_MSG_ERROR([$1])
...
AC_DEFUN([IXCOIN_QT_CHECK],[
  if test "x$ixcoin_enable_qt" != "xno" && test x$ixcoin_qt_want_version != xno; then
    true
    $1
  else
    true
...
dnl IXCOIN_QT_PATH_PROGS([FOO], [foo foo2], [/path/to/search/first], [continue if missing])
...
AC_DEFUN([IXCOIN_QT_PATH_PROGS],[
  IXCOIN_QT_CHECK([
    ...
    if test "x$$1" = "x" && test "x$4" != "xyes"; then
      IXCOIN_QT_FAIL([$1 not found])
    fi
  ])
])

dnl Initialize qt input.
dnl This must be called before any other IXCOIN_QT* macros to ensure that
...
dnl         tried first.
dnl Outputs: See _IXCOIN_QT_FIND_LIBS_*
dnl Outputs: Sets variables for all qt-related tools.
dnl Outputs: ixcoin_enable_qt, ixcoin_enable_qt_dbus, ixcoin_enable_qt_test
AC_DEFUN([IXCOIN_QT_CONFIGURE],[
  ...
  IXCOIN_QT_PATH_PROGS([UIC], [uic-qt${ixcoin_qt_got_major_vers} uic${ixcoin_qt_got_major_vers} uic], $qt_bin_path)
  IXCOIN_QT_PATH_PROGS([RCC], [rcc-qt${ixcoin_qt_got_major_vers} rcc${ixcoin_qt_got_major_vers} rcc], $qt_bin_path)
  ...
  IXCOIN_QT_CHECK([
    ...
    if test x$have_qt_test = xno; then
      ixcoin_enable_qt_test=no
    fi
    ixcoin_enable_qt_dbus=no
    if test x$use_dbus != xno && test x$have_qt_dbus = xyes; then
      ixcoin_enable_qt_dbus=yes
    fi
  ...
  AC_MSG_RESULT([$ixcoin_enable_qt (Qt${ixcoin_qt_got_major_vers})])
  ...
  AC_SUBST(QT_SELECT, qt${ixcoin_qt_got_major_vers})
  AC_SUBST(MOC_DEFS)
])
...
dnl Output: ixcoin_cv_static_qt=yes|no
...
AC_DEFUN([_IXCOIN_QT_IS_STATIC],[
  ...
  if test xixcoin_cv_static_qt = xyes; then
...
AC_DEFUN([_IXCOIN_QT_CHECK_STATIC_PLUGINS],[
...
dnl Inputs: ixcoin_qt_want_version (from --with-gui=). The version to check
dnl         first.
dnl Inputs: $1: If ixcoin_qt_want_version is "auto", check for this version
dnl         first.
dnl Outputs: All necessary QT_* variables are set.
dnl Outputs: ixcoin_qt_got_major_vers is set to "4" or "5".
    ...
    IXCOIN_QT_CHECK([
      ...
      if test x$have_qt == xno && test x$ixcoin_qt_want_version == xauto; then
        if test x$auto_priority_version = x$qt5; then
          ...
          PKG_CHECK_MODULES([QT], [$qt5_modules], [QT_INCLUDES="$QT_CFLAGS"; have_qt=yes; QT_LIB_PREFIX=Qt5; ixcoin_qt_got_major_vers=5], [have_qt=no])
        fi
      fi
      if test x$have_qt != xyes; then
        have_qt=no
        IXCOIN_QT_FAIL([Qt dependencies not found])
      fi
    ])
    IXCOIN_QT_CHECK([
...
dnl Inputs: ixcoin_qt_want_version (from --with-gui=). The version to use.
dnl         If "auto", the version will be discovered by _IXCOIN_QT_CHECK_QT5.
dnl Outputs: All necessary QT_* variables are set.
dnl Outputs: ixcoin_qt_got_major_vers is set to "4" or "5".
dnl Outputs: have_qt_test and have_qt_dbus are set (if applicable) to yes|no.
AC_DEFUN([_IXCOIN_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  IXCOIN_QT_CHECK([AC_CHECK_HEADER([QtPlugin],,IXCOIN_QT_FAIL(QtCore headers missing))])
  IXCOIN_QT_CHECK([AC_CHECK_HEADER([QApplication],, IXCOIN_QT_FAIL(QtGui headers missing))])
  IXCOIN_QT_CHECK([AC_CHECK_HEADER([QLocalSocket],, IXCOIN_QT_FAIL(QtNetwork headers missing))])
