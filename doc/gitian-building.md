...
*Setup instructions for a gitian build of IXCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the IXCoin
...
- [Building IXCoin](#building-ixcoin)
...
Clone the git repositories for ixcoin and gitian,
...
In [doc/release-process.md](release-process.md) in the ixcoin repository under 'Fetch and build inputs'.
...
To build IXCoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the ixcoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ixcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/ixcoin/ixcoin
...
For example:
```bash
URL=https://github.com/laanwj/ixcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit ixcoin=${COMMIT} --url ixcoin=${URL} ../ixcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ixcoin=${COMMIT} --url ixcoin=${URL} ../ixcoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/ixcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ixcoin-build.assert
```
...
[ixcoin/gitian.sigs](https://github.com/ixcoin/gitian.sigs/) repository, or
